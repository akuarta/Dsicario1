...
const CartScreen = ({ navigation }) => {
  ...
  function handleRemoveItem(item) {
    ...
    console.log('LOG BOT├ôN BORRAR:', productId, productName);
    console.log('LOG CANTIDAD:', productQty);
    console.log('LOG PRECIO:', productPrice);

    showConfirm(
      'Remover producto',
      `┬┐Est├ís seguro de que quieres remover ${productName} del carrito?`,
      () => {
  ...
  const handleClearCart = useCallback(() => {
    showConfirm(
      'Vaciar carrito',
      '┬┐Est├ís seguro de que quieres vaciar todo el carrito?',
      clearCart
    );
  }, [clearCart]);
  ...
  const handleCheckout = useCallback(() => {
    if (cart.length === 0) {
      showAlert('Carrito vac├¡o', 'Agrega productos al carrito antes de continuar');
      return;
    }
      ...
      <View style={styles.cartItem}>
        ...
        <View style={styles.itemActions}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => {
              console.log('Bot├│n borrar presionado para:', item.id, item.nombre);
              handleRemoveItem(item);
            }}
            activeOpacity={0.7}
          >
  ...
  const renderEmptyCart = useCallback(() => (
    <View style={globalStyles.emptyContainer}>
      ...
      <Text style={globalStyles.emptyText}>Tu carrito est├í vac├¡o</Text>
      <TouchableOpacity 
