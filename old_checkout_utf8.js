...
const CheckoutScreen = ({ navigation, route }) => {
  ...
  const startWaitingCycle = useCallback((orderId, rider) => {
    clearAllTimers(); // ­ƒº╣ Limpiar CUALQUIER timer anterior para evitar que se vuelva "loco"
    ...
    pollTimerRef.current = setInterval(async () => {
      try {
        ...
        if (estado === 'aceptado' || estado === 'listo' || estado === 'ready') {
          ...
          // OJO: No marcamos ├®xito todav├¡a, esperamos a que el cliente pulse el bot├│n final
          Alert.alert('┬íRider acept├│! ­ƒøÁ', `${selectedRiderRef.current?.nombre} est├í disponible. Ahora puedes confirmar tu pedido.`);
        } else if (estado === 'rechazado') {
          stopWaiting();
          setSelectedRider(null);
          setRiderModalVisible(false);
          Alert.alert('Rechazado ÔØî', 'El repartidor no puede tomar el pedido.');
        }
    ...
    deadlineTimerRef.current = setTimeout(() => {
      ...
      setSelectedRider(null);
      setRiderModalVisible(false);
      Alert.alert('ÔÅ░ Sin respuesta', `${riderName} no respondi├│ a tiempo.`);
  ...
  const sendRiderProposal = async (rider) => {
    try {
      ...
      if (deliveryType === 'delivery' && !deliveryAddress.trim()) {
        Alert.alert('Falta direcci├│n', 'Por favor ingresa la direcci├│n para el repartidor.');
      ...
      // ­ƒÜÇ VELOCIDAD M├üXIMA: Guardar y notificar al mismo tiempo
      const savePromise = saveOrder(orderData);
      
      if (rider) {
        // Enviar notificaci├│n y empezar contador YA
  ...
  const executePayment = async () => {
    try {
      ...
      // CASO 1: El repartidor YA acept├│. Ahora el cliente confirma el trato final con sus datos de pago.
      if (riderConfirmed && currentOrderId) {
        // Recogemos los datos finales de pago que el cliente llen├│
        const finalPaymentData = {
          metodo: paymentType === 'cash' ? 'Efectivo' : 'Tarjeta',
          Pagado: paymentType === 'cash' ? numericAmountReceived : 0,
          Devuelta: paymentType === 'cash' ? devuelta : 0,
          Estado: 'pending' // Ahora s├¡, a cocina
        };
      ...
      if (deliveryType === 'delivery' && !deliveryAddress.trim()) {
        Alert.alert('Falta direcci├│n', 'Por favor ingresa la direcci├│n de entrega.');
        setIsProcessing(false);
        return;
      }
  ...
  const handleGenerateInvoice = async (tipo) => {
    ...
    try {
      const orderData = {
        ...
        NombreLocal: businessInfo?.name || 'D┬┤Sicario', DireccionLocal: businessInfo?.address || 'Rep├║blica Dominicana',
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.successContainer}>
          ...
          <Text style={styles.successTitle}>┬íPedido Exitoso!</Text>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.section}>
          ...
          {deliveryType === 'delivery' && (
            <View style={styles.riderTrustContainer}>
              ...
              <Text style={[styles.sectionTitle, { marginTop: 20, marginBottom: 8, fontSize: typography.sizes.md }]}>Direcci├│n de Entrega</Text>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 15 }}>
                <Text style={{ fontSize: typography.sizes.sm, color: colors.text.primary, fontWeight: 'bold' }}>Tarifa base de env├¡o:</Text>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 15 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  ...
                  <Text style={{ fontSize: typography.sizes.sm, color: colors.text.primary, fontWeight: 'bold' }}>Env├¡o Express (+{formatPrice(costoExpressDelNegocio)})</Text>
                </View>
                <Switch
          ...
          <View style={{ marginTop: 10 }}>
            ...
            {costoEnvioCalculado > 0 && <View style={styles.taxRow}><Text style={styles.taxLabel}>Costo de Env├¡o {isExpressEnvio ? '(Express)' : ''}</Text><Text style={styles.taxValue}>{formatPrice(costoEnvioCalculado)}</Text></View>}
        ...
        <View style={[styles.section, riderConfirmed && { borderColor: colors.success, borderWidth: 2, padding: 15, borderRadius: 20 }]}>
          <Text style={styles.sectionTitle}>
            {riderConfirmed ? '­ƒÅü PASO FINAL: DETALLES DE PAGO' : 'Pago'}
          </Text>
          <TouchableOpacity 
          ...
          {paymentType === 'cash' && (
            <View style={styles.cashInputContainer}>
              <Text style={{ fontSize: 13, color: colors.text.secondary, marginBottom: 10, fontWeight: 'bold' }}>
                {deliveryType === 'delivery' ? '┬┐Con cu├ínto dinero en efectivo enviar├í el pago al repartidor?' : '┬┐Con cu├ínto dinero en efectivo vas a pagar?'}
              </Text>
      ...
      <GlassPanel intensity={20} style={styles.footer}>
        <TouchableOpacity 
          style={[
            ...
            riderConfirmed && { backgroundColor: '#4CAF50' } // Verde si ya acept├│
          ]} 
          disabled={isAmountInsufficient || isProcessing} 
          onPress={executePayment}
        >
          <Text style={styles.confirmButtonText}>
            {riderConfirmed ? '┬íACEPTADO! CONFIRMAR PEDIDO FINAL' : `Confirmar ${formatPrice(finalTotal)}`}
          </Text>
        </TouchableOpacity>
      </GlassPanel>
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView>
              <TouchableOpacity style={styles.riderItem} onPress={() => sendRiderProposal(null)}>
                ...
                <Text style={{ flex: 1, fontWeight: 'bold', color: colors.primary }}>Asignaci├│n autom├ítica</Text>
              </TouchableOpacity>
              {riders.map(r => (
                <TouchableOpacity key={r.id_delivery} style={styles.riderItem} onPress={() => sendRiderProposal(r)}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.riderDetail}>ID: {r.id_delivery} ÔÇó {r.vehiculo}</Text>
                  </View>
