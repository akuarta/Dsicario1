...
const AdminWaiterScreen = ({ navigation }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const waiterStaff = (allUsers || []).filter(u => {
        const role = (u.Rol || u.rol || u.UserType || u.usertype || u.role || '').toLowerCase();
        return role.includes('mesero');
      });
      setStaff(waiterStaff);
      setOrders(allOrders || []);
    } catch (e) {
  ...
  const getStatsForStaff = useCallback((staffEmail) => {
    ...
    const staffOrders = orders.filter(o => {
      const waiterId = (o.id_mesero || '').toLowerCase();
      return waiterId === staffEmail.toLowerCase();
    });
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    staffAvatar: {
      width: 48, height: 48, borderRadius: 24, backgroundColor: '#F1C40F' + '20',
      ...
      <View style={styles.staffCard}>
        <View style={styles.staffHeader}>
          <View style={styles.staffAvatar}>
            <FontAwesome5 name="user" size={22} color="#F1C40F" />
          </View>
          <View style={{ flex: 1 }}>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statBox}>
            <Text style={styles.statNumber}>RD${stats.totalSales.toFixed(0)}</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Meseros</Text>
        <Text style={styles.headerSubtitle}>{staff.length} meseros registrados</Text>
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>No hay meseros registrados</Text>}
        />
      )}
    </SafeAreaView>
  );
